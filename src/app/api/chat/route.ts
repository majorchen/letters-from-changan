...
function normalizePlayerState(value: unknown): PlayerState | null {
  ...
  return {
    ...
    secondCorrespondentHints: Array.isArray(state.secondCorrespondentHints) ? state.secondCorrespondentHints.filter((item): item is string => typeof item === 'string').slice(-8) : [],
    storyTime: {
