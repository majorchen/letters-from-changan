...
### 待验证
- [ ] 信件重复触发是否仍会复现；新邮箱状态机已加入，但旧兼容路径尚未完全移除

### 技术债
- [ ] 状态推进仍有部分 fallback 依赖关键词匹配（客栈/陶器/信等），需继续迁移到 [STATE] 协议
- [ ] 逐步移除 hasMailbox / unreadLetters 旧字段依赖，完全切换到 mailbox 状态机
...
- [x] Prompt 层：重写林深第一封信，必须包含一个"他不应该知道的细节"
- [x] Prompt 层：李无名（算命先生）可加隐含线索，暗示他似乎知道邮箱的存在
- [x] Prompt 层：王掌柜可无意中说一句话，和林深信里的内容产生矛盾
- [x] Prompt 层：林深第三封信开始加入不可靠性（说法和NPC矛盾，玩家需判断谁在说谎）
- [ ] 系统层：把上述钩子沉淀为可追踪事件链，而不只依赖 prompt 自发执行
